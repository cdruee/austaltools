        ...
        for ido, do in enumerate(dirs):
            ...
            if diff_min == 360. or ui is None or vi is None:
                raise ValueError('no reference profile for ' +
                                 'stability class: %s' %
                                 _dispersion.KM2021.num2name(iso + 1))
...
def calc_ref_adapted(levels: list[float], dirs: list[float],
    ...
    pp = {_dispersion.KM2021.num2name(i + 1): u_a_classes[i]
          for i in range(N_CLASS)}
    ...
    for istab in range(N_CLASS):
        for idir, d in enumerate(dirs):
            ...
            if diff_min == 360.:
                raise ValueError('no reference profile for ' +
                                 'stability class: %s' %
                                 _dispersion.KM2021.num2name(istab + 1))
...
def add_options(subparsers):
    ...
    pars_eap.add_argument('-r', '--reference',
                          ...
                          help='choose kind of reference profile.\n'
                               '`general` produces vdi 3783-8 reference'
                               ' wind profiles based on vdi 3783-16'
                               ' geostrophic wind values,\n'
                               '`simple` produces vdi 3783-8 reference'
                               ' wind profiles based on measured wind'
                               ' speed values (needs a timeseries "az"'
                               ' configured in austal.txt),\n'
                               '`austal` runs AUSTAL to create a wind'
                               ' library from which it extracts '
                               ' the reference wind profiles,\n'
                               '`file` reads reference profile from '
                               ' file `Ref1d.dat`.\n'
